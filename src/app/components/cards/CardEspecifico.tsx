...
export default function CardEspecifico(props: CardEspecificoProps) {
  return (
    ...
    >
      {/* Imagem */}
      <figure className="relative bg-cover lg:w-[1000px] group-hover:lg:w-[600px] group-hover:h-[350px] lg:h-[400px]">
        <Image
          src={props.imagem}
      ...
      <div
        className="absolute top-0 left-0 w-full h-full p-1 flex flex-col justify-center gap-4 rounded-lg
          ...
          lg:static bg-fundoCard2 bg-opacity-90 group-hover:opacity-100 lg:bg-transparent"
      >
        <h2
          id={`titulo-${props.titulo}`}
          className="titulo respoFont text-letra2"
        >
          {props.titulo}
        </h2>

        <ul
          className="text-md md:text-[2rem] lg:text-[1rem] text-corLetra list-disc pl-5"
        ...
        <Link
          href="/contato"
          className="bg-letra2 p-1 text-center text-fundo rounded-md shadow-md hover:scale-105 hover:bg-[rgb(159, 226, 24)] transition-all duration-300 text-[1.2rem] md:text-[2rem] lg:text-[1.1rem] respoFont"
